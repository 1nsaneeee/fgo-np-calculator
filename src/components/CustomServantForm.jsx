...
const SectionTitle = ({ children }) => (
  <Typography variant="subtitle2" className="form-section-title">{children}</Typography>
);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: 1 }}>
      ...
      <div>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px 12px' }}>
          ...
          <TextField label="NP4%" type="number" value={sv.np4 || 0} onChange={(e) => set('np4', parseInt(e.target.value) || 0)} size="small" />
          <TextField label="NP5%" type="number" value={sv.np5 || 0} onChange={(e) => set('np5', parseInt(e.target.value) || 0)} size="small" />
        </Box>
      </div>

      <div>
        <SectionTitle>被动技能</SectionTitle>
