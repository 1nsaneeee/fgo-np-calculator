...
function CardBadge({ type, servant }) {
  const colorMap = { B: 'var(--buster)', A: 'var(--arts)', Q: 'var(--quick)', NP: 'var(--card-extra)' };
  return (
...
function SequenceRow({ seq, rank, isKill, hpThreshold }) {
  return (
    <div style={{
      ...
      <span style={{ width: 28, fontSize: 'var(--text-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>
        #{rank}
      </span>
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        {seq.cards.map((c, i) => (
          <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            {i < seq.cards.length - 1 && <span style={{ color: 'var(--text-muted)', fontSize: 'var(--text-2xs)' }}>{'->'}</span>}
          </span>
        ))}
      </div>
      <span style={{ marginLeft: 'auto', fontSize: 'var(--text-sm)', fontWeight: 600, whiteSpace: 'nowrap' }}>
        <span style={{ color: 'var(--arts)' }}>{fmtNum(seq.min)}</span>
        <span style={{ color: 'var(--text-muted)', margin: '0 2px' }}>~</span>
        <span style={{ color: 'var(--color-negative)' }}>{fmtNum(seq.max)}</span>
      </span>
    </div>
  );
}
  ...
  if (!allSequences) {
    return (
      <div className="section-card">
        ...
        <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', textAlign: 'center', padding: 'var(--space-3)' }}>
          请先配置完整的三人组队
        </div>
      </div>
    );
  ...
  return (
    <div className="section-card">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-3)' }}>
        <label style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', fontWeight: 600, whiteSpace: 'nowrap' }}>
          目标 HP
        </label>
        <input
      ...
      {killSequences.length > 0 && (
        <>
          <h3 style={{ fontSize: 'var(--text-sm)', color: 'var(--color-positive)', fontWeight: 700, marginBottom: 'var(--space-1)', display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}>
            致命牌型 (伤害 &ge; {fmtNum(hpThreshold)})
            <span style={{ fontWeight: 400, fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
              共 {killSequences.length} 种
            </span>
          </h3>
          <div style={{ maxHeight: 400, overflowY: 'auto', marginBottom: 'var(--space-3)' }}>
            ...
            {killSequences.length > Math.min(20, showCount) && (
              <button className="toggle-btn" onClick={() => setShowCount(prev => prev + 10)} style={{ width: '100%', marginTop: 'var(--space-1)' }}>
      ...
      <h3 style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', fontWeight: 700, marginBottom: 'var(--space-1)' }}>
        最高伤害前 {Math.min(showCount, topSequences.length)}
      </h3>
